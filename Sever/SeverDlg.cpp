...
BEGIN_MESSAGE_MAP(CSeverDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_START, &CSeverDlg::OnBnClickedStart)
END_MESSAGE_MAP()
...
void CSeverDlg::OnBnClickedStart()
{
	// TODO: 在此添加控件通知处理程序代码
	m_log.ResetContent();  //原有数据清除
	//创建监听套接字,激发FD_ACCEPT事件,默认端口25  
	if (m_SeverSocket.m_hSocket == INVALID_SOCKET)
	{
		BOOL bFlag = m_SeverSocket.Create(UINT(25), SOCK_STREAM, FD_ACCEPT);    //只对FD_ACCEPT事件感兴趣  
		if (!bFlag)
		{
		...
		//监听成功,等待连接请求  
		if (!m_SeverSocket.Listen())//如果监听失败  
		{
			int nErrorCode = m_SeverSocket.GetLastError();    //检测错误信息  
